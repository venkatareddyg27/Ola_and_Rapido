...
RADIUS_EXPANSION_STEPS = [5, 10, 15, 20]
RATING_WEIGHT = 0.3


def driver_score(distance: float, rating: float, radius_km: float) -> float:
    normalized_distance = distance / radius_km
    normalized_rating = 1.0 - (rating / 5.0)

    return ((1 - RATING_WEIGHT) * normalized_distance) + (
...
def bounding_box(lat: float, lng: float, radius_km: float):
    ...
    lng_delta = radius_km / (111.320 * cos(radians(lat)))
...
class DriverMatchingService:
    ...
    def __init__(self, db: AsyncSession, redis_client=None, websocket_manager=None):
        self.db = db
    ...
    async def find_nearby_drivers(
        ...
        result = await self.db.execute(
            ...
            .join(DriverLocation, DriverLocation.id == latest_location_subquery)
            .where(
                DriverProfile.status == DriverStatus.ONLINE,
                DriverProfile.is_verified == True,
                DriverLocation.is_active == True,
                Vehicle.category == vehicle_category.upper(),
        ...
        rows = result.all()
        print(f"Found {len(rows)} nearby drivers")
    ...
    async def send_ride_requests(
        ...
        print(f"Auto assigned driver: {first_driver['driver_id']}")
    ...
    async def accept_ride(
        ...
        print(f"Trip assigned to driver {driver_id}")
        return True
    ...
    async def reject_ride(
        ...
        print(f"Driver rejected trip: {driver_id}")
        return True
    ...
    async def match_driver(
        ...
        print(f"Searching drivers for {vehicle_category}")

        for radius_km in RADIUS_EXPANSION_STEPS:
            print(f"Searching in radius {radius_km} KM")
            ...
            if not nearby_drivers:
                print(f"No drivers in {radius_km} KM")
                continue
