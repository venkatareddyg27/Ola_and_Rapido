...
class TripInvoice(Base):
    ...
    driver_id = Column(UUID(as_uuid=True),ForeignKey("driver_profiles.id", ondelete="CASCADE"),nullable=False)
    ...
    trip_type = Column(SqlEnum(TripType),nullable=True)
    ...
    surge_charge = Column(Numeric(10, 2),default=0)
    ...
    invoice_status = Column(SqlEnum(InvoiceStatus),default=InvoiceStatus.generated)
    ...
    customer = relationship("User",back_populates="trip_invoices")
    driver = relationship("DriverProfile",back_populates="trip_invoices")
    vehicle = relationship("Vehicle",back_populates="trip_invoices")
    rental = relationship("Rental",back_populates="invoice")
    rented_vehicle = relationship("RentedVehicle",back_populates="trip_invoices")